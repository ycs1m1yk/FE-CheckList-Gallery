...
function Tags({ category }: ICategoryListProps) {
  const tagResult = category.map((tag) => (
    <TagLink key={tag.children} to={`/gallery?tag=${tag.category._id}`}>
      {tag.category.name}
    </TagLink>
  ));

  return <TagBox>{tagResult}</TagBox>;
}
...
function CodeContainer({ code }) {
  ...
  function CodeEditors() {
    const codes = code.map((file) => {
      ...
      return (
        <div>
          <p>{file}</p>
          <StyledCodeEditor openState={openState} value={text} padding={20} language={type} readOnly />
        </div>
      );
    });
  ...
  return (
    <div>
      <CodeToggle openState={openState} onClick={handleToggle}>
        {openState ? '코드 접기' : '코드 보기'}
      </CodeToggle>
      <CodeEditors />
    </div>
  );
...
function Content({ code }) {
  return <ContentBox>{code}</ContentBox>;
}
